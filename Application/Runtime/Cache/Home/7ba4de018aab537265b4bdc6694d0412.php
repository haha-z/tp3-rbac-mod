...
<div class="container-fluid">
  ...
  <div class="col-xs-3 col-sm-3 col-md-3  col-lg-3">
    <a href="<?php echo U('Access/set');?>" class="btn btn-link pull-right"><button type="button" class="btn btn-info">添加用户</button></a>
  </div>
</div>
<hr/>
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr id="<?php echo ($v["id"]); ?>">
          ...
          <td>
              <a href="<?php echo U('Access/set',array('uid'=>$v['id']));?>">编辑</a>
  ...
  function del(id) {
    ...
    if (confirm_) {
      $.ajax({
        ...
        url:"<?php echo U('Home/Access/delete');?>",
        async:true,
        timeout:5000,    //超时时间
        ...
        success:function(data, textStatus){
          ...
          if (data.status == 1 ) {
            $("#"+id).remove();
            window.location.href = "<?php echo U('Home/Access/index');?>";
          }
        },
