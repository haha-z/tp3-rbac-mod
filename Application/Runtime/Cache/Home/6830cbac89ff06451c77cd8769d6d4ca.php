...
<div class="container-fluid">
  ...
  <div class="col-xs-3 col-sm-3 col-md-3  col-lg-3">
    <a href="<?php echo U('User/set');?>" class="btn btn-link pull-right"><button type="button" class="btn btn-info">添加用户</button></a>
  </div>
</div>
<hr/>
  ...
  <table class="table table-bordered">
    ...
    <tbody>
      <?php if(is_array($list)): foreach($list as $key=>$v): ?><tr id="<?php echo ($v["id"]); ?>">
          ...
          <td>
            <?php if($v["status"] == 1): ?><b>启用 </b>
            <?php elseif($v["status"] == 0): ?> <b>禁用</b><?php endif; ?>
          </td>
          <td>
            <?php if($v["is_admin"] == 1): ?><b>是</b>
            <?php elseif($v["is_admin"] == 0): ?> <b>否</b><?php endif; ?>
          </td>
          <td>
              <a href="<?php echo U('User/set',array('uid'=>$v['id']));?>">启用 </a>
              <a href="<?php echo U('User/set',array('uid'=>$v['id']));?>">编辑</a>
